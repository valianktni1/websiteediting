...
OUT = "/app/wpbuild/plugin/ivory-digital-elementor"
TPL = os.path.join(OUT, "templates")
# token maps to the PLUGIN BASE url; markup keeps its own /assets/ path
ASSET_TOKEN = "__IVORY_BASE__"
...
def rewrite_assets(html):
    # prefix absolute/relative /assets or assets with the base token (keeps /assets/)
    html = html.replace('="/assets/', '="%s/assets/' % ASSET_TOKEN)
    html = html.replace('="assets/', '="%s/assets/' % ASSET_TOKEN)
    html = html.replace("url('/assets/", "url('%s/assets/" % ASSET_TOKEN)
    html = html.replace('url("/assets/', 'url("%s/assets/' % ASSET_TOKEN)
    return html

def html_widget(markup):
    return {"id": nid(), "elType": "widget", "widgetType": "html",
            "settings": {"html": rewrite_assets(markup)}, "elements": []}
...
def full_container(child):
    s = {
        ...
        "flex_gap": {"unit": "px", "size": 0, "sizes": [], "column": "0", "row": "0"},
...
def extract_seo(soup):
    ...
    jsonld = [str(s) for s in head.find_all('script', type='application/ld+json')]
...
for path in sorted(glob.glob(os.path.join(SRC, "*.html"))):
    ...
    for child in body.find_all(recursive=False):
        ...
        if child.name == 'main':
            ...
            if secs:
                for sec in secs:
                    content.append(full_container(html_widget(str(sec))))
            else:
                content.append(full_container(html_widget(child.decode_contents())))
        else:
    ...
    doc = {"content": content,
           ...
           "version": "0.4",
           "title": (seo_map[slug]['title'] or slug),
           "type": "page"}
    with open(os.path.join(TPL, slug + ".json"), "w", encoding='utf-8') as f:
        f.write(url_clean(json.dumps(doc)))

    manifest[slug] = {"title": seo_map[slug]['title'], "source": fname}
    order.append(slug)

with open(os.path.join(OUT, "data", "manifest.json"), "w", encoding='utf-8') as f:
    json.dump({"order": order, "pages": manifest}, f)
with open(os.path.join(OUT, "data", "seo.json"), "w", encoding='utf-8') as f:
    f.write(url_clean(json.dumps(seo_map, ensure_ascii=False)))

for extra in ("robots.txt", "llms.txt", "llms_full.txt", "sitemap.xml"):
    p = os.path.join(SRC, extra)
    if os.path.exists(p):
        shutil.copy(p, os.path.join(OUT, "data", extra))
...
print("pages:", len(order), "| token:", ASSET_TOKEN)
